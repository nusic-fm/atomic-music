...
async function main() {
  ...
  const marketPlaceBalance = await masterContract.marketPlaceBalance();
  console.log("marketPlaceBalance = ",marketPlaceBalance.toString());

  const addr2UserBalance = await masterContract.userBalance(addr2.address);
  console.log("addr2UserBalance = ",addr2UserBalance.toString());
