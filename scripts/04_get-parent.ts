...
async function main() {
  ...
  let tokenId = 0;
  const ownerAddress = await atomicMusicNFT.ownerOf(tokenId);
  console.log(`Owner of TokenId ${tokenId} is ${ownerAddress}`);
  const nftParentAddress = await atomicMusicNFT.nftOwnerOf(tokenId);
  console.log(`NFT Parent of TokenId ${tokenId} is ${nftParentAddress}`);
}
