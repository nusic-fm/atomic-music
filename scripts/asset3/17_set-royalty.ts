...
async function main() {
  ...
  const txt = await atomicMusicMCNFT.setTokenRoyalty(2, owner.address, BigNumber.from(2000));
  console.log("setTokenRoyalty txt.hash = ",txt.hash);
  const txtReceipt = await txt.wait();
  console.log("setTokenRoyalty txt.hash = ",txtReceipt);
